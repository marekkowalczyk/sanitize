...
- **Pre-scan collision detection** — highest priority safety feature (dev/BACKLOG.md). Build old→new mapping, detect duplicate targets, abort before any renames. Should be done before recommending `-r` for production use on large trees. Consider doing this before v1.0.0 if scope allows.
