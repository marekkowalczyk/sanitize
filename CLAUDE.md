...
```bash
go build .                          # build
go test -v                          # run full test suite (150+ cases)
...
- Polish `l`/`L` and German `ss` are special-cased with direct string replacement because they are standalone characters, not combining sequences
...
- Idempotency tests
- CLI integration tests (stdin, args, exit codes, `-f` mode, `san` symlink)
...
- `CODE-REVIEW.md` -- Code review with issue tracker
